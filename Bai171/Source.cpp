...
int main()
{
	...
	int lt, rt, lb, rb;
	TimConToanDuong(a, m, n, lt, rt, lb, rb);
...
bool checkToanDuong(float a[][500], int &lt, int &rt, int &lb, int &rb)
{
	for (int i = lt; i <= lb; i++)
		for (int j = lt; j <= rt; j++)
			if (a[i][j] < 0)
...
void TimConToanDuong(float a[][500], int m, int n, int &lt, int &rt, int &lb, int &rb)
{
	int max = 0;
	lt, rt, lb, rb = -1;
		...
		for (int width = 1; width <= n; width++)
			for (int i = 0; i <= m - height; i++)
				for (int j = 0; j <= n - width; j++)
				{
					...
					int rb_check = j;
					if (checkToanDuong(a, lt_check, rt_check, lb_check, rb_check))
					{
